...
- **feat (closes #145):** nova preferência "Abrir tour guiado automaticamente" (grupo Movimento, padrão ligado): desligada, o tour só abre em Ajuda › Rever tour guiado — um tour que cobre a tela sozinho é mudança de contexto não pedida para quem usa leitor de tela. Com "Reduzir movimento" (SO ou toggle), o Joyride não usa beacon pulsante em nenhum passo, rola até o alvo sem animação (`scrollDuration` 0: a rolagem dele é em JS, a regra CSS global não alcança) e abre o tooltip sem fade. A regra global `html.reduzir-movimento *` (#149) já zerava animações e transições CSS, e a rolagem para seção já lia o store — o teste confirma nenhuma animação/transição > 0,01 ms. **Animação feita em JS (bibliotecas, `scrollIntoView`) lê `reduzirMovimento` do store; a regra CSS só cobre CSS.**
- **feat (closes #146):** toda rota tem exatamente um `<h1>` visível, em qualquer largura, descrevendo a tela, e nenhum salto de nível (auditoria em 32 rotas a 390 e 1280 px + axe `heading-order`/`page-has-heading-one`). **Telas de acesso:** o `<h1>` é o objetivo da tela ("Entrar", "Redefinir senha"…) e fica no `<main>`; o slogan do `AuthBrandPanel` virou `<p>` — o painel some abaixo do `md`, e com ele sumia o único `<h1>`. **Painel:** `DashboardLayout` ganhou `tituloNoConteudo` — a tela traz o próprio `<h1>` (nome do paciente/aluno, "Cadastrar TAF", "Atendimentos realizados") e o nome do painel no cabeçalho vira texto. Telas sem título de bloco ganharam `<h2>` (visível quando já havia rótulo, como "Sua posição"/"Efetivo geral"/nome no card de atendimento; `sr-only` onde o design não tem título). `TermoConsentimentoLGPD` recebe `nivel`; títulos de modal são `<h2>`; tabela de importação com `<caption>` e `scope="col"`. **Tela nova: um `<h1>` que descreve a tela e `<h2>` por bloco; título dentro de `<button>` perde a semântica — ponha-o antes.**
- **feat (closes #147):** campo obrigatório é sinalizado **antes** do envio — padrão único do app em `components/CampoObrigatorio.jsx`: `<MarcaObrigatorio />` (asterisco `aria-hidden`) no rótulo + `aria-required="true"` no campo + `<AvisoObrigatorios />` ("Campos marcados com * são obrigatórios") no topo do formulário. Aplicado nos 12 formulários (acesso, senha, onboarding, perfil, cadastros, TAF, modais, "Fale com o time"); o teste confere que enviar vazio dá erro **exatamente** nos campos marcados. Formato esperado sai do placeholder para texto de ajuda persistente ligado por `aria-describedby`: CPF no Login, telefone de emergência e datas dd/mm/aaaa — `fieldErrorProps(erro, nome, { dica: true })` junta `${nome}-dica` e `${nome}-erro`. "Fale com o time" ganhou `autoComplete` name/organization/email. No caminho: flexões/abdominais/barra do TAF vazios passavam como 0 (`z.coerce.number("")`); agora dão erro. **Campo novo obrigatório: asterisco + `aria-required` + schema que rejeita vazio; formato esperado vai em `<p id="x-dica">`, não só no placeholder.**
