...
- **test (closes #123):** primeiro teste automatizado do `frontend/`. Vitest + Testing Library + jsdom, `npm test` (e `npm run test:watch`); configuração no bloco `test` do `vite.config.js`, setup em `src/test/setup.js`. **135 testes** cobrindo o que quebra em silêncio e custa caro: validação de CPF por dígito verificador e mascaramento (regra de design — CPF nunca aparece completo para quem não é o dono), senha forte com a garantia de que `avaliarSenha` e `senhaForteSchema` **não divergem** (se divergirem, a barra fica verde numa senha que o formulário recusa), datas inexistentes/futuras, faixas de peso e altura (issue #41 — "1.70" no campo de centímetros), neutralização de **CSV injection** no histórico exportado e de **open redirect** em `rotaInternaSegura` (#107, que desde a #129 também sanitiza destino de notificação). `features/auth/RotaProtegida.jsx` tem a matriz completa: os 4 papéis contra as rotas dos outros 3, verificando que o acesso é negado **e** que a pessoa é levada à própria tela inicial (não a um 404). Somado um teste de regressão da #128 — papel semeado à mão no `localStorage` não restaura sessão com o modo demo desligado. **Os testes foram verificados contra sabotagem deliberada do código que cobrem** (remover a guarda da #128 quebra 1 teste; anular a guarda de papel quebra 25) — um teste que passa com e sem a correção não protege nada. Plugar no CI continua sendo a issue #68.
- **chore:** `npm audit` voltou a zero. Advisory novo de severidade alta em `fast-uri` (transitiva de `serve` → `ajv`, já presente no repositório antes desta issue, não trazida pelas dependências de teste) corrigido com `npm audit fix`, sem quebra. O Vitest entrou direto na major 5 porque a 3/4 arrasta `@vitest/mocker` com path traversal conhecido.
